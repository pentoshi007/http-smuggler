...
## 10. Major Concurrency Bottleneck
**Severity:** High
**File(s):** `http_smuggler/core/engine.py`

**Description:**
While detection *payloads* might be async in some places, the core endpoint scanning loop in `engine.py` is sequential:
```python
for endpoint in endpoints:
    # ...
    await self._test_endpoint(...)
```
If a scan discovers 100 endpoints and each takes 1 minute to scan (conservative for timing tests), the scan will take nearly 2 hours. `_test_endpoint` waits for all payloads to finish before moving to the next URL.
This makes the tool unusable for large-scale crawling and scanning.

**Recommendation:**
Refactor to use a worker pool (semaphore) to scan multiple endpoints concurrently (e.g., 5-10 at a time), similar to how the crawler works.

## 11. Inefficient Connection Usage (No Keep-Alive)
**Severity:** Medium
**File(s):** `http_smuggler/detection/timing.py`

**Description:**
The `TimingDetector` opens a **new TCP connection** for every single baseline request and every single payload:
```python
async with AsyncRawHttpClient(self.network) as client:
    await client.connect(...)
```
This is extremely wasteful ("noisy") and significantly slows down the scan. It also increases the chance of hitting server-side rate limits or firewall "SYN flood" protections.
A "Best Class" tool should reuse connections (HTTP Keep-Alive) where possible, especially for baseline checks.

**Recommendation:**
Implement connection pooling or reuse the `client` session across the `measure_baseline` loop and `detect_batch` where safe.

## 12. Missing TCP_NODELAY in Async Client
**Severity:** Medium
**File(s):** `http_smuggler/network/raw_socket.py`

**Description:**
The synchronous `RawHttpClient` correctly sets `TCP_NODELAY` (disabling Nagle's algorithm). However, the `AsyncRawHttpClient` does not:
```python
# AsyncRawHttpClient.connect
self._reader, self._writer = await asyncio.wait_for(asyncio.open_connection(...))
# Missing: socket configuration
```
Timing-based smuggling often relies on sending small chunks (e.g., "Q") and measuring delay. If `TCP_NODELAY` is not set, the OS might buffer this small chunk, causing false negatives in timing tests.

**Recommendation:**
Retrieve the socket object from `writer.get_extra_info('socket')` and explicitly set `TCP_NODELAY`.

## 13. Lack of False Positive "Verification" Phrase
**Severity:** Medium
**File(s):** `http_smuggler/core/engine.py`

**Description:**
If `TimingDetector` returns a positive result (vulnerability found), the engine immediately reports it (if differential is disabled or fails).
Real-world networks are jittery. A single spike in latency can look like a timeout.
The tool lacks a "Verify" step that re-runs the *specific* successful timing payload 2-3 times to confirm consistent behavior before reporting.

**Recommendation:**
Add a `verify_timing_vulnerability` step in the engine that re-tests a candidate payload multiple times to filter out network noise.

## 14. Missing "Reproduce Scan" Features
**Severity:** Low (Usability)
**File(s):** `http_smuggler/analysis/reporter.py`

**Description:**
The reports (JSON/Markdown) contain the raw payload but do not provide an easy "copy-paste" command (like `curl` or `netcat`) to reproduce the finding manually. This is a standard feature in "best-in-class" tools (like Burp Suite or ZAP).

**Recommendation:**
Add a "Reproduction" field to the report generating a one-liner `printf ... | nc ...` or python script snippet to verify the bug manually.
